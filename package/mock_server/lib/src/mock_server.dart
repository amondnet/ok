...
/// A `Dispatcher` is used to customize the responses of the `MockServer`
/// further than using a queue.
///
/// Using the `Dispatcher` will invalidate all the default response values
/// of the `MockServer` so be sure to set an `httpCode`.
///
/// It is called with an `HttpRequest` object every time the `MockServer`
/// receives a request.
///
...
/// Defines a set of values that the `MockServer` will return to a given
/// request. Used with `MockServer.enqueueResponse(MockResponse response)` or
/// a `Dispatcher`.
class MockResponse {
  Object body;
  int httpCode;
...
/// Contains the info of a request received by the MockServer instance.
class StoredRequest {
  String body;
  String method;
  Uri uri;
...
///
/// The simplest way of using the `MockServer` is to script the session with a
...
class MockServer {
  ...
  Future<void> start() async {
    var address = _addressType == InternetAddressType.IPv4
